...
void on_login_clicked(GtkButton *button, UserData *userData) {
    ...
    {
        printf("login NULL\n");
        gtk_label_set_text(GTK_LABEL(userData->ScreenApp->loginContainer.show_login_status), "Input all password and username");
//        gtk_widget_set_visible(userData->ScreenApp->loginContainer.show_login_status,TRUE);
    ...
    else
    {
        if(loginUser(userData->sockFd,userData) ==1){
            ...
            gtk_label_set_text(GTK_LABEL(userData->ScreenApp->mainContainer.show_player_name), userData->playerName);
            ...
            gtk_label_set_text(GTK_LABEL(userData->ScreenApp->loginContainer.show_login_status),"Wrong User or Password");
...
//    memset(username,'\0',strlen(username)+1);
//    memset(password,'\0',strlen(password)+1);
    gtk_entry_set_text(GTK_ENTRY(userData->ScreenApp->loginContainer.username_input),"");
    gtk_entry_set_text(GTK_ENTRY(userData->ScreenApp->loginContainer.password_input),"");
...
//            gtk_widget_hide(userData->ScreenApp->loginContainer.login_window);
//            gtk_widget_show(userData->ScreenApp->mainContainer.main_window);
            gtk_label_set_text(GTK_LABEL(userData->ScreenApp->loginContainer.show_login_status), "sign up success");


        }else{
            gtk_label_set_text(GTK_LABEL(userData->ScreenApp->loginContainer.show_login_status), "Register fail");
          //  gtk_widget_set_visible(userData->ScreenApp->loginContainer.show_login_status,TRUE);
        }
